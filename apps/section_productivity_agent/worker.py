"""Worker for section productivity analytics output generation."""
...
from analytics.phase3 import build_section_productivity, write_section_productivity_json
...
class SectionProductivityAgentWorker:
    """Generate one section productivity analytics artifact."""
...
def process_work_item(work_item: WorkItem, overwrite: bool = False) -> AgentResult:
    ...
    if not isinstance(branch, str) or not isinstance(report_date, str):
        return _failure_result("branch and report_date are required for section productivity analytics.")

    analytics_payload = build_section_productivity(branch, report_date, root=root)
    output_path = write_section_productivity_json(
        analytics_payload,
